    ...
    if (messageToSend != "") {
      ...
      //local - http://127.0.0.1:5000/user
        ...
        .then((res) => {
          ...
          ch.push({ from: "our", msag: messageToSend });
...
renderJobSearchResources = () => {

  // check if chatbot's last response is "Here are a few online resources that can help you with your job search: LinkedIn, Glassdoor, Indeed, CareerBuilder, Monster. Remember to tailor your job search to your goals and interests, and consider seeking help from a career coach or mentor if needed."
  const lastResponse = this.state.chat[this.state.chat.length - 1].msag;
...
renderCareerGuidanceChoiceButtons = () => {
  // check if chatbot's last response is "You are at the right place! I can guide you to the right resource"
  const lastResponse = this.state.chat[this.state.chat.length - 1].msag;
...
renderWellBeingGuidanceChoiceButtons = () => {
  // check if chatbot's last response is "You are at the right place! I can guide you to the right resource"
  const lastResponse = this.state.chat[this.state.chat.length - 1].msag;
...
endChat = () => {
  ...
  render() {
    return (
        ...
        <Header />
          <div id="msger" className="msger">
            <div id="b-container" className="bubble-container">
              <div id= "left-msg" className="msg left-msg">
                <div className="msg-img"></div>
                <div className="msg-bubble">
                    <div className="msg-info">
                      <div className="msg-info-name">StriveBot</div>
      ...
      {/* Conditionally render chatDelay gif and greeting message based on showChatDelay state variable */}
      {this.state.showChatDelay ? (
        <div className="msg left-msg"> <div className="msg-img">
        {" "}
      </div>
          <div className="msg-bubble">
            <div className="msg-info">
        ...
        {" "}
                      ...
                      {this.renderWellBeingGuidanceChoiceButtons()}
                      {this.renderAppsforWellBeing()}
                      {this.renderJobSearchResources()}
                      {this.linkInterviewPrepArticle()}
                      {this.linkResumeWritingArticle()}
                      {this.linkCoverLetterArticle()}
                      {this.linkManageStressArticle()}  
                      {this.getActiveArticleandWorkOuts()}
                      {this.anxietyArticleandBreathing()}
                      {this.endChat()}
                    </div>
                  );
                } else {
